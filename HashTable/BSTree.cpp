...
int BSTree<DataType, KeyType>::getHeightHelper(BSTreeNode * p) const
{
	...
	{
		int l = getHeightHelper(p->left); //repeatedly update l
		int r = getHeightHelper(p->right); //repeatedly update r
		if (l > r) // print out the one with the biggest height
			return (++l);
		else
			return (++r);
	}
...
{
	int  i = 1;
	if (p != NULL) //set an integer to 1 to account for root
	{
		if (p->left != NULL) //if there is a left node go to it and count it
			i += getCountHelper(p->left);
		if (p->right != NULL) //same as left but with right
